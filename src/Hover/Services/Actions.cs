...
public static class Actions
{
    ...
    /// The image trays, so a preference that moves the deck also moves the tray to
    /// the opposite edge.
    public static Hover.Images.ImageStripManager? ImageStrips { get; set; }
    ...
    public static void SetTabFont(string family)
    {
        Settings.TabFontName = family;
        Refresh();
    }

    public static void SetCodeFont(string family)
    {
        Settings.CodeFontName = family;
        Refresh();
    }
    ...
    /// Points the tray at a different folder. An empty string means the default. Moves
    /// no existing file.
    public static void SetShotsFolder(string folder)
    {
        Settings.ShotsFolder = folder;
        Hover.Images.ShotStore.Shared.Rebind();
        Refresh();
    }

    /// Zero keeps everything. Removed files go to the Recycle Bin.
    public static void SetShotRetention(int days)
    {
        Settings.ShotRetentionDays = days;
        Hover.Images.ShotStore.Shared.TidyNow();
        Refresh();
    }
    ...
    public static ContextMenu BuildMainMenu(DeckController? deck = null)
    {
        ...
        foreach (var f in Ink.Favourites)
        {
            var captured = f;
